...
router.post("/:chatid", (request, response) => {
    ...
    let insert = ` Select email from members where memberid in
    (select memberid_b from contacts where memberid_a=$1 and memberid_b not in (select memberid from chatmembers where chatid=$2) and verified = 1);`
    console.log(request.body)
